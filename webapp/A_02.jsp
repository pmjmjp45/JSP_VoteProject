...
<body>
	...
	<%
		...
		List<VoteItem> showDeleted= voteItemDao.deleteCandidates(delete);
		
		for (int i = 0; i < showDeleted.size(); i++) {
	%>
		<tr>
			<td><%=showDeleted.get(i).getNumber() %></td>
			<td><%=showDeleted.get(i).getName() %></td>
			<td>
				<form method="get" action="A_02.jsp">
					<button type="submit" name="delete" value= <%=showDeleted.get(i).getNumber() %>>삭제</button>
				</form>
			</td>
		</tr>
	...
	<%} 
		int num = 1;
		for (int i = 0; i <showDeleted.size(); i++) {
			if ((i + 1) != showDeleted.get(i).getNumber()) {
				num = i + 1; 
				break;
			}
			num++;
