...
<body>
	...
	%>
		<tr>
			<td><%=showAll.get(i).getNumber() %></td>
			<td><%=showAll.get(i).getName() %></td>
			<td>
				<form method="get" action="A_02.jsp">
					<button type="submit" name="delete" value= <%=showAll.get(i).getNumber() %>>삭제</button>
				</form>
			</td>
		</tr>
	<%} 
		int num = 1;
		for (int i = 0; i <showAll.size(); i++) {
			if ((i + 1) != showAll.get(i).getNumber()) {
				num = i + 1; 
				break;
			}
			num++;
